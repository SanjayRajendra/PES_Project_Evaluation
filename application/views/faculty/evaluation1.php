...
<html> 
<head> 
    <title>Projects Evaluation_1</title> 
    ...
    <script>
        function restore1(id,srn)
        {
            ...
            document.getElementById(titl).disabled=false;
...
<div class="container"> 
    ...
    <div class="form-group"> 
        <ul class="nav nav-tabs">
            <li class="active"><a href="#">Synopsis Approval</a></li> 
    ...
    <div>
        ...
        <small style="margin-left: 2%;color:#eeeeee"><b><?php echo "$r->name";?></b></small></h2>
    ...
    <?php foreach ($records1 as $value) 
    {
        ...
        echo form_open_multipart('faculty/evaluation1_submit',array('class' => 'form-horizontal'));
        echo "
        <input type='text' name='srn' value='".$value->srn."' style='display:none'>
        <table width='100%' class='table' >
        <tr>    
        <td><label>Concept of the work / Originality & Significance & Relevance – Application in Real world<br><small>(Max. Marks = 10)</small></label>
        <input class='form-control'type='number' min='0' max='10' name='realworld' id='realworld".$value->srn."' onchange='sum_marks(this.id,\"$value->srn\")' required></td>
        <td><label>Title:<br><small>(Max. Marks = 5)</small></label>
        <input class='form-control' min='0' max='5'type='number' name='title' id='title".$value->srn."' onchange='sum_marks(this.id,\"$value->srn\")'  required></td>
        </tr>
        <tr>
        <td>
        <label>Methodology Proposed:<br><small>(Max. Marks = 5)</small></label>
        <input class='form-control' min='0' max='5'type='number' name='method' id='method".$value->srn."' onchange='sum_marks(this.id,\"$value->srn\")'  required></td>
        ...
        <tr>
        <td></td>
        <td><input type='submit' name='submit' class='btn btn-default' id='submit".$value->srn."' style='margin-left: 30%' onclick='fun(this.id,\"$value->srn\");'>
        <input type='reset' name='reset' id='reset".$value->srn."' onclick='restore1(this.id,\"$value->srn\")' class='btn btn-default'></td>
        </tr>
        </table>";
        echo form_close();
        echo "</div>";
