    ...
    <script>
        function get_model(pid) 
        {
          $('#modal-fullscreen').html('');
           $.ajax(
                {
                  url:'<?php echo base_url();?>admin/project_pop',
                  method:'post',
                  data:{'pid':pid},
        ...
        function success(pid) 
        {
           $.ajax(
                {
                  url:'<?php echo base_url();?>admin/project_approved',
                  method:'post',
        ...
        function reject(pid)
        {
           $.ajax(
                {
                  url:'<?php echo base_url();?>admin/project_reject',
                  method:'post',
                  data:{'pid':pid,'comment':$('#comment').val()},
                  success:function(response)
                  {
    ...
    <div class="page-header"> 
        ...
        <?php include_once 'sidenav.php';?>
            ...
            foreach ($records as $v) 
            {
                ...
                echo "<td><button type='button' class='btn btn-link' id='button1' data-toggle='modal' data-target='#modal-fullscreen' onclick=get_model('$v->id');>Verify</button>
